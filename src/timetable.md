...
### B-1. Julia入門から論文投稿まで(仮)
- 講師: TBA
- 概要: Juliaは「使いやすさと速さの両立」を成し遂げた革新的なプログラミング言語である。たびたび議題に挙げられる速度や使いやすさといったメリットはJuliaの本質を捉えておらず、これらの性質を両立する点こそが計算科学における多言語問題の解決に必要不可欠である。2025年にはGoogle ColabによるJuliaの公式サポートが開始し、APSおよびSIAMがJuliaConの開催パートナーに加わるなど、学術用途における有用性が高く評価されており、今後の発展にも大きな期待が寄せられている。そこで、本講義では、計算物理学においてJuliaを全面的に活用している研究者を招聘し、Juliaの環境構築から入門、数値計算への応用例などを解説するほか、実際の論文投稿を見据えたパッケージ開発および研究での活用方法を解説する. ※ 講師が決定次第, 詳細は変更される場合があります. (文責: 世話人・大野周平)
- 事前知識: なし
...
### C-1. GPUによる数値線形代数
- 講師: 丹 愛彦 (NVIDIA)、他（To Be Announced）
- 概要: 格子QCD計算、原子核構造計算、原子・分子・固体の電子状態計算など、量子力学で記述される現象の具体的な計算には行列固有値問題が現れる。このように、計算物理学の多くの課題には連立一次方程式や行列固有値問題が現れ、計算時間の主要因となることが多く、その短縮にGPUを用いた並列計算が有効である。そこで、本講義枠では大手GPUメーカーから講師を招聘し、行列積や対称行列固有値問題などを題材とした講義とハンズオンを行う。参加者各自がOpenBLAS/LAPACKとGPUライブラリによるサンプルコード動かしながら並列化効率を計測し、GPUアクセラレーションを体験する。※講師が決定次第, 詳細は変更される場合があります。（文責：世話人・大野周平）
