...
::: danger 講義の締め切りについて
一部の講義は席数の関係上募集を締め切ることがあります。ご了承ください。
(2025/12/18 0:00追記)
:::

## グループセミナー

グループセミナーは6人程度の参加者が集まって1グループを作り、グループ内で自身の研究や興味を持っている話題について発表し合う企画です。1人あたり10分程度、スライドを見せながら話すことを想定しています。様々な分野からの参加が見込まれるため、専門外の方が聞くことも念頭に入れた構成でお願いします。自分の興味や関心のあるテーマ、気になっている論文などの、カジュアルな発表についても歓迎いたします。
...
- 講師: 金賀 穂（千葉大）
- TA: ~~大野 周平（横浜市大/理研）~~、品岡 寛（埼玉大）、水野 航希（名古屋大）
- 概要: Julia は「使いやすさと速さの両立」を志向して設計されたプログラミング言語である。2025年には Google Colab による Julia の公式サポートが開始されたほか、APS DCOMP や SIAM が JuliaCon 2025 に参画するなど、今後の発展が期待されている。
本講義では、強力なコーディング能力をもつ生成AIを「相棒」として、科学計算をJuliaで行うための基礎を学ぶ。  
    前半3コマは品岡（大野担当分を代講）・水野が担当し、Julia の環境構築から出発して、GitHub を起点とする開発フローを体験する。短い題材コードを用いて、型・多重ディスパッチ・broadcast といった Julia の抽象化を読み解きつつ、生成AIの提案をテストと計測で検証しながら改良していく流れを実践する。
    また、前半の最後にスピントロニクス分野への適用例として水野がTensor Closs Interpolationの方法を紹介し、その簡単な実装を体験する。  
    後半2コマは金賀が担当し、量子マスター方程式を用いた高次高調波発生の解析をハンズオンで体験する。配布する雛形コードを起点として、生成AIを活用しながら部品となる関数を実装・検証・改善し、研究ソフトウェア開発と数値解析の実践的手法を学習する。進捗に応じて、テストやドキュメント整備、パッケージ公開に向けた手順の概略についても紹介する。
...
### C-1. GPUによる数値線形代数
- 講師: 丹 愛彦（NVIDIA）、石橋 史康 (AMD)、大原 久樹 (AMD)
...
- 事前知識: CLIの基本操作、行列とベクトルの基礎、C言語によるプログラミングの経験

::: danger 注意
以降の申し込みを行う参加者にはこちらからスパコン環境を用意することができませんのでご留意ください。  
(2026/2/10)
:::
...
- 事前知識: 本講義を理解するためには、量子力学の基礎的な知識を有していることが望ましい。 また、書籍「密度汎関数法の基礎」 (KS物理専門書／常田 貴夫 著)を事前に読んでおくことを推奨する。
